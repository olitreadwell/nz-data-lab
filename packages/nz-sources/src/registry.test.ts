...
import { getNzDataSource, probeAllNzDataSources, probeNzDataSource } from './registry';
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (url.includes('nzor')) {
          return new Response(readFixtureText('nzor-names-kiwi.xml'), { status: 200 });
        }
        if (url.includes('linz')) {
          return jsonResponse(readFixtureJson('linz-property-titles.json'));
        }
