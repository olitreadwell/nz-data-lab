...
export function parseCsv(text: string): Array<Record<string, string>> {
  ...
  return rows.slice(1).map((values) => {
    if (values.length > header.length) {
      throw new StatsNzParseError('CSV row has more fields than the header row');
    }
...
export function parseStatsNzCsv(
  ...
  if (firstRecord === undefined || !Object.keys(firstRecord).includes('OBS_VALUE')) {
  ...
  for (const record of records) {
    ...
    if (value !== null && !Number.isFinite(value)) {
      throw new StatsNzParseError(`Invalid OBS_VALUE in Stats NZ CSV: ${rawValue}`);
    }
...
export function serializeStatsNzRowsToCsv(rows: StatsNzObservation[]): string {
  ...
  const dimensionKeys = [...new Set(rows.flatMap((row) => Object.keys(row.dimensions)))].sort();
